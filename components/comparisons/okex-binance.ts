...
// export function getOkexComparison(binanceOrderbooks: any, okexOrderbooks: any): RowInfo[] {
...
//         const okexData = okexOrderbooks.usdtPairs[symbol.toLowerCase()];

//         if (!binanceData || !okexData) continue;

//         // Check if we have valid bid/ask
//         if (!binanceData?.bid?.length || !binanceData?.ask?.length || 
//             !okexData?.bid?.length || !okexData?.ask?.length) continue;
...
//             const rowInfo = createOkexRowTable(
//                 okexData.ask,
//                 binanceData.ask,
//                 difference_percent,
//                 okex_quantity,
